...
def compute_composite_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['RSI_BullRange'] = (df['RSI14'] >= 48).astype(int)
    df['RSI_BearRange'] = (df['RSI14'] <= 45).astype(int)
    df['RSI_Divergence_Flag'] = 0
    df['ATR_Breakout_Flag']  = (df['ATR14'] >= df['ATR14'].rolling(20).mean() * 1.5).astype(int)
    df['Vol_Expansion_Flag'] = (df['VolumeRatio20'] >= 1.2).astype(int)
    df['Tech_Confirm'] = (
        (df['Close'] > df['MA200']) &
        (df['Close'] > df['MA50']) &
        (df['RSI14'] >= 48) &
        (df['VolumeRatio20'] >= 1.2)
    ).astype(int)
